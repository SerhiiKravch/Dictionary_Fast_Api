...
@router.get("/", response_model=MessageResponse, status_code=200)
def index() -> MessageResponse:
    return MessageResponse(message="Dictionary FastAPI is running")


@router.post("/lookup", response_model=WordRead, status_code=200)
...
@router.get("/word/{slug}", response_model=WordRead, status_code=200)
